...
Save the raw transcript under `artifacts/<task-id-or-slug>/data/`.
...
Derive a concise `name` and a clear `description` for **each** chapter based **only** on its transcript.
